...
func (a *Adapter) RegisterLogger(logger logging.Logger) { a.app.RegisterLogger(logger) }
...
func (a *Adapter) handleRoute(request *server.Request[any]) (any, error) {
	...
	if request.Action == string(protocol.ApiUploadCreate) {
		...
		params := formToMultipartParams(form)
...
func (a *Adapter) forwardNative(account *client.Account, origin *http.Request, endpoint string) (*server.Response, error) {
	...
	if origin != nil {
		...
		options = append(options, client.WithRequestHeaders(protocol.NativeRequestHeaders(origin.Header)))
	}
...
func formToMultipartParams(files server.UploadCreateParam) map[string]any {
	params := make(map[string]any, len(files))
	for name, file := range files {
		params[name] = client.NewUpload(file.Data, file.Filename, file.ContentType)
	}
	return params
}
